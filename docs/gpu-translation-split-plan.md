...
**目标**: 将翻译管线拆分为 detect/translate/render 三阶段，支持 GPU 和 Gemini 解耦调度，提升吞吐量和稳定性。

> ⚠️ **数据修正 (2026-02-14)**: 原方案估算 `detect~2s + render~0.5s = 2.5s/页` 严重偏低。
> 实测 detect 含完整模型推理（目标检测+OCR），render 含 mask+inpaint+text_rendering 全流程。
> 实测数据见 §8。

**状态**: GPU Worker 端点已落地 + 联调通过；82 后端 split 协调逻辑已实现并进入收口阶段
...
用户请求 → 82 后端 → PPIO GPU Worker (RTX 4090)
                     ...
                     总服务端 (unified): 34,446ms
                     端到端:             46,900ms

⚠️ 注: 上述 104ms 仅为 text_rendering 子步骤。
   _complete_translation_pipeline 全流程 (mask+inpaint+rendering) 实测 11-16s。
```
...
> ✅ **当前状态 (2026-02-14)**: Phase2 已在 82 后端实现。翻译通过 `_translate_texts_for_split()`
> 在 82 executor 进程内执行，并通过 `Semaphore(1)` 串行保护全局 translator。
...
```
              时间轴 →                                                  (实测数据)
GPU Worker:   [detect_1 13s][idle ~30s gemini][render_1 14s][detect_2 13s][idle ~30s]...
...
**主要收益 (实测):**
- Split GPU 总耗时: ~27s/页 vs unified ~41s/页 → **~1.5x 提升** (Cloud Run L4)
- PPIO 4090: split ~21.5s vs unified ~35.8s → **~1.7x 提升**
- 单次请求时间缩短 → 请求超时风险降低 → **稳定性提升**
...
**freeTimeout 优化 (2026-02-14):**

> `freeTimeout` 从 300s 降至 **60s**。
> 原因: PPIO 按实例时长计费, 空闲保活期间照常计费。300s 空闲在翻译 10 页场景下浪费占比 58%。
> 降至 60s 后: 10 页场景浪费占比降至 ~22%, 且 60s 足够覆盖用户连续操作间隔。
> 代价: 超过 60s 未有新请求将触发冷启动 (~90s), 但使用频率低时这是合理权衡。
...
|------|---------|---------|---------------|
| x-regions-count | ✅ | ✅ | ✅ (已有) |
| x-stage-elapsed-ms | `{"detect":N,"ocr":null,"total":N,"mode":"aggregated_detect_ocr"}` | `{"render":N}` | `{"context":N,"render":N}` (已有) |
...
基于 **2026-02-14 联调实测数据** (001.jpg, 16 regions):

### PPIO RTX 4090 实测

| 指标 | unified (实测) | split (实测) | 改善 |
|------|:-------------:|:-----------:|:----:|
| detect | — | 10.3-13.3s | — |
| render | — | 11.2-14.1s | — |
| GPU 总耗时/页 | 35.8s | 21.5-27.3s | **~1.5-1.7x** |

### Cloud Run L4 实测

| 指标 | unified (实测) | split (实测) | 改善 |
|------|:-------------:|:-----------:|:----:|
| detect | — | 13.2s (热) / 28.9s (冷) | — |
| render | — | 14.1-16.1s | — |
| GPU 总耗时/页 | 41.4s | 27.3s | **~1.5x** |
| 每页成本 | $0.013 | $0.0088 | **~1.5x** |

### 跨平台对比

| 平台 | GPU | split GPU 总计 | unified | 提升 |
|------|-----|:--------------:|:-------:|:----:|
| PPIO | RTX 4090 | 21.5s | 35.8s | 1.7x |
| Cloud Run | NVIDIA L4 | 27.3s | 41.4s | 1.5x |

> **核心收益是吞吐量与稳定性, 不是成本。**
> Cloud Run 按秒计费 + 缩容到 0，比 PPIO (含 freeTimeout 浪费) 便宜约 55%。
>
> ⚠️ **数据修正说明**: 原方案估算 `detect~2s + render~0.5s = 2.5s/页, 14x 提升`
> 来自对 context 阶段 104ms render 子步骤的错误理解。104ms 仅为 text_rendering，
> `_complete_translation_pipeline` 全流程 (mask+inpaint+rendering) 实测 11-16s。
