...
**下载后的使用方法**：
1. 下载 `models` 文件夹
2. 将 `models` 文件夹放到程序根目录（与 `app.exe` 或 `步骤2-启动Web服务.bat` 同级）
3. 启动程序即可使用
...
**手动部署版本**：
- 运行命令：`python -m manga_translator web`
...
**文件位置**：`examples/filter_list.txt`（程序启动时自动创建，或点击设置中的"打开过滤列表"按钮）

**使用方法**：
1. 在设置面板中点击"**打开过滤列表**"按钮，或手动打开 `examples/filter_list.txt`
2. 在对应区域添加过滤词（不区分大小写）
3. 以 `#` 开头的行为注释
4. 保存文件后，下次翻译时自动生效
...
- `[包含过滤]`：原文**包含**这些文本就过滤（模糊匹配）
- `[精确过滤]`：原文必须**完全等于**这些文本才过滤（精确匹配）
